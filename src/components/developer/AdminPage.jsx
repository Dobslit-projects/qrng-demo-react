...
function formatDate(iso, lang) {
  if (!iso) return "-";
  return new Date(iso).toLocaleString(lang === "en" ? "en-US" : "pt-BR", { day: "2-digit", month: "2-digit", year: "2-digit", hour: "2-digit", minute: "2-digit" });
}
...
function TokensTab({ tokens, onRefresh }) {
  ...
  async function handleRevoke(id) {
    if (!confirm(t("adminRevokeTokenConfirm"))) return;
    setBusy(id + "-revoke");
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, fontFamily: mono }}>
        <thead>
          <tr>
            {["e-mail", "prefix", "status", t("adminThQuotaDay"), t("adminThReqToday"), t("adminThCreated"), t("adminThActions")].map((h) => (
        ...
        <tbody>
          {tokens.length === 0 && (
            <tr><td colSpan={7} style={{ padding: "16px 10px", textAlign: "center", color: theme.textMuted }}>{t("adminNoTokens")}</td></tr>
          )}
          {tokens.map((tok, i) => (
            <tr key={tok.id} style={{ background: i % 2 === 0 ? "transparent" : theme.surfaceAlt }}>
              <td style={{ padding: "8px 10px", color: theme.textDim, whiteSpace: "nowrap" }}>{tok.email || "—"}</td>
              <td style={{ padding: "8px 10px", color: theme.quantum, whiteSpace: "nowrap" }}>{tok.token_prefix}</td>
              <td style={{ padding: "8px 10px" }}>
                <span style={{ fontSize: 10, fontWeight: 700, fontFamily: mono, padding: "2px 8px", borderRadius: 12, background: tok.status === "active" ? theme.success + "18" : theme.danger + "18", color: tok.status === "active" ? theme.success : theme.danger, border: `1px solid ${tok.status === "active" ? theme.success + "40" : theme.danger + "40"}` }}>
                  {tok.status}
                </span>
              </td>
              <td style={{ padding: "8px 10px", color: theme.textDim }}>
                {editingId === tok.id ? (
                  <div style={{ display: "flex", gap: 4 }}>
                    ...
                    />
                    <button onClick={() => handleQuotaSave(tok.id)} disabled={busy === tok.id + "-quota"} style={{ padding: "2px 8px", borderRadius: 4, border: `1px solid ${theme.success}40`, background: theme.success + "14", color: theme.success, fontFamily: mono, fontSize: 10, cursor: "pointer" }}>OK</button>
                  ...
                  <span style={{ cursor: "pointer", textDecoration: "underline dotted" }} onClick={() => { setEditingId(tok.id); setQuotaInput(String(tok.quota_daily)); }}>{tok.quota_daily}</span>
                )}
              </td>
              <td style={{ padding: "8px 10px", color: theme.textDim }}>{tok.requests_today}</td>
              <td style={{ padding: "8px 10px", color: theme.textMuted, whiteSpace: "nowrap" }}>{formatDate(tok.created_at, lang)}</td>
              <td style={{ padding: "8px 10px" }}>
                {tok.status === "active" && (
                  <button
                    onClick={() => handleRevoke(tok.id)}
                    disabled={busy === tok.id + "-revoke"}
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, fontFamily: mono }}>
        <thead>
          <tr>
            {["id", "e-mail", t("adminThRole"), t("adminThRegisteredAt")].map((h) => (
              <th key={h} style={{ textAlign: "left", padding: "6px 10px", color: theme.textMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", fontSize: 10, borderBottom: `1px solid ${theme.border}`, whiteSpace: "nowrap" }}>
                {h}
              </th>
            ))}
        ...
        <tbody>
          {users.length === 0 && (
            <tr><td colSpan={4} style={{ padding: "16px 10px", textAlign: "center", color: theme.textMuted }}>{t("adminNoUsers")}</td></tr>
          )}
          {users.map((u, i) => (
            <tr key={u.id} style={{ background: i % 2 === 0 ? "transparent" : theme.surfaceAlt }}>
              ...
              <td style={{ padding: "8px 10px", color: theme.textMuted, whiteSpace: "nowrap" }}>{formatDate(u.created_at, lang)}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div style={card}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 14, fontWeight: 700, color: theme.text, fontFamily: mono }}>{t("adminPanelTitle")}</span>
        <button
          onClick={loadData}
      ...
      <div style={{ display: "flex", gap: 4 }}>
        {[
          { id: "tokens", label: `${t("adminTabTokens")} (${tokens.length})` },
          { id: "users",  label: `${t("adminTabUsers")} (${users.length})` },
        ].map((tabItem) => (
          <button
            key={tabItem.id}
            ...
            style={{
              ...
              background: activeTab === tabItem.id ? theme.surfaceAlt : "transparent",
